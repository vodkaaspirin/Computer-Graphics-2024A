...
// Copyright: 2015 Guangdong University of Technology. All rights reserved.
// 
...
//			Possess it if you can.		能拥有之便是一种幸福。
//			Seldom found in woman.		在女性之中这很少见，
//			Never found in man.			在男性之中更是寥寥无几。
// 
//			― Jonathan Morris, The Way of Serenity: Finding Peace and Happiness in the Serenity Prayer
//				乔纳森·莫里斯 《宁静之道：在宁静祈祷中找到平和与幸福》
// 
///////////////////////////////////////////////////////////////////////////////
...
float _camy = 60.0f;			// 摄像机位置y
float _camz = 150.0f;			// 摄像机位置z
float _tgtx = 0.0f;				// 摄像机目标x
float _tgty = 0.0f;				// 摄像机目标y
float _tgtz = 0.0f;				// 摄像机目标z

bool _fullscreen = true;		// 全屏显示
...
"Guangzhou Tower Model by Li Guanxiao,3122010037,School of International Education,Guangdong University of Technology\n"
...
void init()
{
	...
	// 添加0号灯
	GLfloat LightAmbient[] = { 0.5f, 0.5f, 0.5f, 1.0f };		// 环境光颜色
	GLfloat LightDiffuse[] = { 0.5f, 0.5f, 0.5f, 1.0f };		// 散射光颜色
	GLfloat LightSpecular[] = { 0.9f, 0.9f, 0.9f, 1.0f };		// 高光颜色
...
void draw_guangzhoutower() {

	float scale = 1.0f;
	glLineWidth(5.0f); // 设置线宽

	float time = glutGet(GLUT_ELAPSED_TIME) / 500.0f; // 转换为秒

	float angles[24] = {
		0.0f, 15.0f, 30.0f, 45.0f, 60.0f, 75.0f, 90.0f, 105.0f, 120.0f, 135.0f,// 定义角度数组
		150.0f, 165.0f, 180.0f, 195.0f, 210.0f, 225.0f, 240.0f, 255.0f, 270.0f,
		285.0f, 300.0f, 315.0f, 330.0f, 345.0f
	};

	glPushMatrix();
	glScaled(1.0f * scale, 1.0f * scale, 1.0f * scale);

	for (int i = 0; i < 24; i++) {					//使用for循环生成物体
		glPushMatrix();

		float r = (sin(time + i * 0.25f) * 0.5f + 0.5f);
		float g = (sin(time + i * 0.25f + 2.0f) * 0.5f + 0.5f); // 相位偏移 2.0
		float b = (sin(time + i * 0.25f + 4.0f) * 0.5f + 0.5f); // 相位偏移 4.0

		glColor3f(r, g, b);

		glRotated(angles[i], 0.0f, 1.0f, 0.0f);		//循环生成24根外立面钢管
		glTranslated(2.0f, 0.0f, 0.0f);
		glRotated(84.0f, 1.0f, 0.0f, 0.0f);
		glScaled(0.7f, 0.7f, 90.0f);
		glutSolidCube(1.0f);
		glPopMatrix();

		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i], 0.0f, 1.0f, 0.0f);
		glTranslated(8.0f, -44.0f, 0.0f);
		glScaled(3.0f, 3.0f, 3.0f);
		glutSolidSphere(0.25f, 24, 24);
		glPopMatrix();

		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i] + 5.0f, 0.0f, 1.0f, 0.0f);
		glTranslated(10.0f, -44.5f, 0.0f);
		glScaled(3.0f, 3.0f, 3.0f);
		glutSolidSphere(0.25f, 24, 24);
		glPopMatrix();
		...
		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i] + 10.0f, 0.0f, 1.0f, 0.0f);
		glTranslated(12.0f, -45.0f, 0.0f);
		glScaled(3.0f, 3.0f, 3.0f);
		glutSolidSphere(0.25f, 24, 24);
		glPopMatrix();
		...
		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i] + 15.0f, 0.0f, 1.0f, 0.0f);
		glTranslated(14.0f, -45.0f, 0.0f);
		glScaled(3.0f, 3.0f, 3.0f);
		glutSolidSphere(0.25f, 24, 24);
		glPopMatrix();
		...
		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i] + 20.0f, 0.0f, 1.0f, 0.0f);
		glTranslated(16.0f, -45.0f, 0.0f);
		...
		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i] + 25.0f, 0.0f, 1.0f, 0.0f);
		glTranslated(18.0f, -45.0f, 0.0f);
		...
		glPushMatrix();
		glColor3f(r, g, b);							//塔底周边球体
		glRotated(angles[i] + 30.0f, 0.0f, 1.0f, 0.0f);
		glTranslated(20.5f, -45.0f, 0.0f);
		glScaled(6.0f, 6.0f, 6.0f);
		glutSolidSphere(0.25f, 24, 24);
		glPopMatrix();
	}
	...
	glTranslated(0.0f, 0.0f, -47.0f);
	glScaled(0.4f, 0.4f,0.3f);
	glutSolidCone(60.0f, 13.0f, 48, 1);
	glPopMatrix();

	glPushMatrix();									//塔上部中心填充
	glColor3f(1.0f, 1.0f, 1.0f);
	glRotated(90.0f, 1.0f, 0.0f, 0.0f);
	glTranslated(0.0f, 0.0f, -44.0f);
	glScaled(0.075f, 0.075f, 5.0f);
	glutSolidCone(-60.0f, 10.0f, 48, 1);
	glPopMatrix();

	glPushMatrix();									//塔上部中心填充2
	glColor3f(1.0f, 1.0f, 1.0f);
	glRotated(90.0f, 1.0f, 0.0f, 0.0f);
	glTranslated(0.0f, 0.0f, -44.0f);
	glScaled(0.075f, 0.075f, -0.2f);
	glutSolidCone(-60.0f, 10.0f, 48, 1);
	glPopMatrix();
	...
	glRotated(90.0f, 1.0f, 0.0f, 0.0f); 
	glScaled(0.4f, 0.4f, 70.0f);
	glutSolidCube(1.0f);
	glPopMatrix();

	glPushMatrix();									//塔尖附加物体
	glColor3f(1.0f, 0.0f, 0.0f);
	glTranslated(0.0f, 55.0f, 0.0f);
	glRotated(90.0f, 1.0f, 0.0f, 0.0f);
	glScaled(0.6f, 0.6f, 5.0f);
	glutSolidCube(1.0f);
	glPopMatrix();

	glPushMatrix();									//塔尖附加物体2
	glColor3f(1.0f, 1.0f, 1.0f);
	glTranslated(0.0f, 55.0f, 0.0f);
	glRotated(90.0f, 1.0f, 0.0f, 0.0f);
	glScaled(1.0f, 1.0f, 0.7f);
	glutSolidCube(1.0f);
	glPopMatrix();

	glPushMatrix();									//塔尖附加物体2
	glColor3f(1.0f, 1.0f, 1.0f);
	glTranslated(0.0f, 57.0f, 0.0f);
	glRotated(90.0f, 1.0f, 0.0f, 0.0f);
	glScaled(1.0f, 1.0f, 0.4f);
	glutSolidCube(1.0f);
	glPopMatrix();

	glPushMatrix();									//塔尖附加球体
	glColor3f(1.0f, 1.0f, 1.0f);
	glTranslated(0.0f, 60.0f, 0.0f);
